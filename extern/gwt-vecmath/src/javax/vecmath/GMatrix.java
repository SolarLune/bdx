...
public class GMatrix implements java.io.Serializable, Cloneable {
	...
	public final void copySubMatrix(int rowSource, int colSource, int numRow,
		...
		if (this != target) {
			for (i = 0; i < numRow; i++) {
				for (j = 0; j < numCol; j++) {
					target.values[rowDest + i][colDest + j] = values[rowSource + i][colSource
							+ j];
				}
			}
		} else {
	 ...
	 * whose matrix positions were eliminated will be lost.
	 * 
	 * @param nRow
	 ...
	 * The values are copied in one row at a time, in row major fashion. The array
	 ...
	 * = U*W*transpose(V); and returns the rank of this matrix; the values of
	 * U,W,V are all overwritten. Note that the matrix V is output as V, and not
	...
	public final int SVD(GMatrix U, GMatrix W, GMatrix V) {
		...
		if (nRow == 2 && nCol == 2) {
			if (values[1][0] == 0.0) {
				...
				compute_2X2(values[0][0], values[0][1], values[1][1], single_values,
	 ...
	 * matrix decompostion of this matrix; the diagonal elements of L (unity) are
	 * not stored. The GVector parameter records the row permutation effected by
	 * the partial pivoting, and is used as a parameter to the GVector method
	 ...
	 * vector "row_perm[]" is an output parameter that contains the row
	 ...
	 * and treats it as the right-hand side of the matrix equation Ax = LUx = b.
	 * The solution vector replaces the original column of the matrix.
	 * 
	...
	static int computeSVD(GMatrix mat, GMatrix U, GMatrix W, GMatrix V) {
		...
		if (m.nRow == 2 && m.nCol == 2) {
			...
			compute_2X2(single_values[0], e[0], single_values[1], single_values,
					sinl, cosl, sinr, cosr, 0);
	...
	static void compute_qr(int start, int end, double[] s, double[] e, GMatrix u,
			GMatrix v) {
		...
		if (Math.abs(e[1]) < CONVERGE_TOL) {
			compute_2X2(s[start], e[start], s[start + 1], s, sinl, cosl, sinr, cosr,
					0);
			e[start] = 0.0;
			e[start + 1] = 0.0;
		} else {
		...
		if (fhmn == 0.0) {
			...
			if (ga < fhmx) {
				...
				if (au == 0.0) {
					...
					d__1 = as * au;
					d__2 = at * au;
					c = 1.0 / (Math.sqrt(d__1 * d__1 + 1.0) + Math
							.sqrt(d__2 * d__2 + 1.0));
					ssmin = fhmn * c * au;
					ssmin += ssmin;
				}
